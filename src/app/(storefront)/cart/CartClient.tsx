    ...
    return (
      <div className="min-h-[70vh] flex flex-col items-center justify-center text-center px-4 bg-[#fcfcfc]">
        <motion.div 
          ...
          className="w-32 h-32 bg-gray-50 rounded-none flex items-center justify-center mb-8 border border-gray-100"
        >
        ...
        >
          <BackButton label={settings.continueShoppingText?.[lang] || 'Continue Shopping'} className="bg-brand-ink text-white px-10 py-4 rounded-none font-medium text-sm uppercase tracking-wide hover:bg-gray-800 transition-all inline-flex items-center group" />
        </motion.div>
      </div>
    );
  }
        ...
        <div className="flex flex-col lg:flex-row gap-12 lg:gap-20">
          <div className="flex-1">
            <div className="space-y-8">
              {items.map((item, index) => (
                ...
                >
                  <div className="w-24 h-32 sm:w-32 sm:h-40 rounded-none bg-gray-50 overflow-hidden flex-shrink-0 border border-gray-100 relative">
                  ...
                  <div className="flex-1 flex flex-col justify-between py-1">
                    <div className="flex justify-between items-start">
                      <div>
                        <h3 className="text-[12px] font-bold uppercase tracking-widest text-brand-ink mb-1.5">{item.translations?.[lang]?.title || item.title}</h3>
                        <p className="text-sm text-brand-muted font-medium">{formatPrice(item.price || 0, lang, item.prices)}</p>
                      </div>
                      <button onClick={() => removeItem(item.id!)} className="w-10 h-10 rounded-none bg-gray-50 flex items-center justify-center text-brand-muted hover:text-red-500 hover:bg-red-50 transition-colors">
                        <Trash2 className="w-4 h-4" />
                      </button>
                    </div>
                    <div className="flex flex-col sm:flex-row sm:items-center justify-between mt-6 gap-4">
                      <div className="flex items-center border border-gray-200 rounded-none overflow-hidden bg-white w-fit">
          ...
          <div className="w-full lg:w-[400px] shrink-0">
            <div className="py-8 border-b border-gray-200/60 last:border-0 sticky top-32">
              ...
              <Link href={`/${lang}/payment`} className="w-full bg-brand-ink text-white px-8 py-4 rounded-none font-medium text-sm uppercase tracking-wide hover:bg-gray-800 transition-all flex items-center justify-center group">
