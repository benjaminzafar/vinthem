          ...
          >
            {availableLanguages.map((lng) => (
              <button
                ...
                disabled={isPending}
                onClick={() => changeLanguage(lng)}
                className={`flex items-center justify-between w-full px-5 py-2.5 text-xs font-bold uppercase tracking-widest transition-colors ${currentLocale === lng ? "text-slate-900 bg-slate-50" : "text-slate-400 hover:text-slate-900 hover:bg-slate-50"}`}
              >
                <span>{lng}</span>
