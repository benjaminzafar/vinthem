    ...
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-[100] p-4">
      <div className="bg-white rounded-[4px] shadow-none border border-zinc-200 w-full max-w-5xl max-h-[90vh] flex flex-col overflow-hidden">
        ...
        <div className="px-5 sm:px-8 py-4 border-b border-zinc-200 bg-white flex justify-between items-center shrink-0">
          ...
          <button 
            onClick={onClose} 
            className="p-2 text-zinc-500 hover:text-zinc-900 transition-colors bg-zinc-50 hover:bg-zinc-100 rounded-md"
          >
            <X className="w-5 h-5" />
          </button>
        </div>
        ...
        <div className="px-5 sm:px-8 py-4 border-t border-zinc-200 bg-zinc-50/30 flex flex-col sm:flex-row sm:items-center justify-between gap-3 shrink-0">
           <p className="text-[12px] font-medium text-zinc-500">Select an asset from your Cloudflare R2 storage.</p>
            <button 
              onClick={onClose}
              className="px-6 h-10 bg-black text-white rounded-[4px] text-[12px] font-medium hover:bg-zinc-800 transition-all"
            >
              Dismiss
            </button>
        </div>
