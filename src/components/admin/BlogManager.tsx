...
export function BlogManager() {
  ...
  const handleAIAutoComplete = async () => {
    ...
    try {
      ...
      let detailedMessage = '';
      if (status === 429 || status === 503 || errorMessage.toLowerCase().includes('quota') || errorMessage.toLowerCase().includes('overload')) {
        detailedMessage = `## Error Type\nConsole Error\n\n## Error Message\n[${timestamp}] AI Generation failed (${status || 'Service Error'}). ${errorMessage}`;
      } else {
        detailedMessage = `## Error Type\nConsole Error\n\n## Error Message\n[${timestamp}] AI AutoComplete failed. ${errorMessage}`;
      }
