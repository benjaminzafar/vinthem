...
import { CheckCircle2, ChevronRight, Share2, ShieldCheck, ShoppingBag, Star, X } from 'lucide-react';
            ...
            <div className="grid grid-cols-4 gap-3 sm:grid-cols-5">
              {allImages.map((image, index) => (
                <button
                  ...
                  className={`relative aspect-[4/5] overflow-hidden border transition-all ${activeImage === image ? 'border-slate-900' : 'border-slate-200 hover:border-slate-400'}`}
                >
          ...
          <div className="border border-slate-200 bg-white p-6 sm:p-8">
            <div className="flex flex-wrap items-center gap-3">
              {product.categoryName && (
                <span className="border border-slate-200 px-3 py-1 text-[11px] font-black uppercase tracking-[0.2em] text-slate-500">
                  {product.categoryName}
                </span>
              )}
              {product.sku && (
            ...
            <div className="mt-6 flex flex-wrap items-center gap-4 text-sm text-slate-500">
              <div className="flex items-center gap-1 text-amber-500">
                {[...Array(5)].map((_, index) => (
                  <Star key={index} className={`h-4 w-4 ${index < Math.floor(product.rating || 5) ? 'fill-current' : ''}`} />
                ))}
              </div>
            ...
            <div className="mt-8 flex flex-wrap items-end gap-4 border-b border-slate-100 pb-8">
              ...
              <div className={`inline-flex items-center gap-2 border px-3 py-2 text-[11px] font-black uppercase tracking-[0.18em] ${currentStock > 0 ? 'border-emerald-200 bg-emerald-50 text-emerald-700' : 'border-rose-200 bg-rose-50 text-rose-700'}`}>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {option.values.map((value) => (
                        <button
                          ...
                          className={`border px-4 py-3 text-[11px] font-black uppercase tracking-[0.18em] transition-all ${selectedOptions[option.name] === value ? 'border-slate-900 bg-slate-900 text-white' : 'border-slate-200 bg-white text-slate-700 hover:border-slate-400'}`}
                        >
                          {value}
                        </button>
                      ))}
            ...
            <div className="mt-8 flex flex-col gap-3 sm:flex-row">
              <button
                ...
                className="flex min-h-[54px] flex-1 items-center justify-center gap-3 bg-slate-900 px-8 text-[11px] font-black uppercase tracking-[0.22em] text-white transition-all hover:bg-slate-800 disabled:cursor-not-allowed disabled:bg-slate-200 disabled:text-slate-400"
              >
              ...
              <Link
                href="/cart"
                className="flex min-h-[54px] items-center justify-center border border-slate-200 px-6 text-[11px] font-black uppercase tracking-[0.22em] text-slate-700 transition-all hover:border-slate-900 hover:text-slate-900"
              >
            ...
            <div className="mt-8 flex items-center gap-3 border-t border-slate-100 pt-6">
              ...
              <button onClick={() => handleShare('WhatsApp')} className="flex h-10 w-10 items-center justify-center border border-slate-200 text-slate-700 transition-all hover:border-slate-900 hover:text-slate-900"><FaWhatsapp /></button>
              <button onClick={() => handleShare('Telegram')} className="flex h-10 w-10 items-center justify-center border border-slate-200 text-slate-700 transition-all hover:border-slate-900 hover:text-slate-900"><FaTelegramPlane /></button>
              <button onClick={() => handleShare('Facebook')} className="flex h-10 w-10 items-center justify-center border border-slate-200 text-slate-700 transition-all hover:border-slate-900 hover:text-slate-900"><FaFacebook /></button>
            </div>
          </div>
          ...
          <div className="grid grid-cols-2 gap-4 md:grid-cols-4 md:gap-8">
            {relatedProducts.filter((item) => Boolean(item?.id)).map((item) => (
              <Link key={item.id} href={`/product/${item.id}`} className="group">
                <div className="relative aspect-[4/5] overflow-hidden border border-slate-200 bg-slate-50">
                  {item.imageUrl ? (
      ...
      <AnimatePresence>
        {isLightboxOpen && activeImage && (
          <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="fixed inset-0 z-[100] bg-slate-950/90 p-4 md:p-10">
            <button onClick={() => setIsLightboxOpen(false)} className="absolute right-6 top-6 z-[110] flex h-12 w-12 items-center justify-center border border-white/20 text-white transition-all hover:border-white/60">
              <X className="h-5 w-5" />
            </button>
