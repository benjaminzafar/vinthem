...
export function ProductClient({
  ...
  const isColorOption = (optionName: string) => {
    ...
    return ['color', 'colour', 'f├ñrg', 'farve', 'farge', 'kleur'].some((keyword) => normalized.includes(keyword));
  };
            ...
            {allImages.length > 1 ? (
              <div className="mt-4 grid grid-cols-4 gap-2 sm:gap-3">
                {allImages.slice(0, 4).map((image, index) => (
                  ...
                  >
                    <Image
                      src={getOptimizedImageUrl(image, 200, 75)}
                          ...
                          {option.values.map((value, valueIndex) => {
                            ...
                            return (
                              ...
                              >
                                {variantImage ? (
                                  <Image
                                    src={getOptimizedImageUrl(variantImage, 120, 75)}
              ...
              {relatedProducts.filter((item) => Boolean(item?.id)).slice(0, 4).map((item) => (
                <Link key={item.id} href={`/${lang}/product/${item.id}`} className="group block">
                  <div className="relative aspect-[4/5] overflow-hidden bg-[#f4f4f2]">
                    {item.imageUrl ? (
                      <Image
                        src={getOptimizedImageUrl(item.imageUrl, 400, 80)}
                        alt={item.title}
                        fill
          ...
          {isLightboxOpen && activeImage ? (
            ...
            >
              ...
              <div className="relative h-full w-full">
                <Image
                  src={getOptimizedImageUrl(activeImage, 1600, 90)}
                  alt={displayTitle}
                  fill
                  sizes="100vw"
                  className="object-contain"
