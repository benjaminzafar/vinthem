...
2026-04-18 | **Light & Accent Filter Design Pivot** | **FIXED MISTAKE**: Replaced the 'heavy' black solid backgrounds in the mobile filter drawer with a premium Scandinavian aesthetic using soft tints and crisp borders, resolving the user's dissatisfaction with the overused dark colors and restoring a minimalist gallery feel.
2026-04-18 | **Mobile Filter Standard: Height & Icon Sync** | **FIXED MISTAKE**: Resolved the 'Expression expected' build error in `MobileFilters.tsx` by cleaning up duplicated tags; integrated custom collection icons from Admin settings with automated fallback logic; and standardized all filter/sort link heights to a compact 48px for perfect visual symmetry.
